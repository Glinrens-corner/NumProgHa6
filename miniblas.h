 ...
 *
 *  @author Steffen B&ouml;rm, <boerm@math.uni-kiel.de>
 *
 *  @{ */
...
/** @brief Copy a vector, @f$y \gets x@f$.
 *
...
HEADER_PREFIX void
copy(unsigned int n, const real *x, unsigned int incx,
 ...
 *
 *  @param lower Boolean value determining if only the lower triangular
 *         part or only the right triangular part should be
 *         updated. Value is true for the lower triangular part, false for
 ...
 *  @param ldA Leading dimension of @f$A@f$. */
HEADER_PREFIX void
syr(bool lower, unsigned int n, real alpha,
    const real *x, unsigned int incx,
    real *A, unsigned int ldA);
...
HEADER_PREFIX void gemm(bool transA, bool transB, unsigned int rows, unsigned int cols, unsigned int k, real alpha,
